...
def copyFull(src, dst):
	...
	status = subprocess.run(["rsync", "-arvh", src, dst], capture_output=True)
	if status.returncode == 0:
		msg = (status.stdout).decode(encoding="UTF-8", errors="ignore")
		files = msg.replace("\r", "").split("\n")
		return messagebox.showinfo(title="Files copied successfully", message=f"{files[-3]}\n\n{files[-2]}")
...
def main():
	...
	tk.Label(root, text="Source:").place(x=25, y=150)

	tk.Label(root, text="Destiny:").place(x=25, y=200)

	tk.Entry(root, width=55, state=tk.DISABLED, textvariable=src).place(x=85, y=150)
	tk.Entry(root, width=55, state=tk.DISABLED, textvariable=dst).place(x=85, y=200)
